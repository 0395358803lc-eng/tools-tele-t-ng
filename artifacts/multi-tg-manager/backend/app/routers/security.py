...
from ..schemas import Bulk2faIn, SecurityMessageOut, TgSessionOut
...
async def mark_all_read(
    account_id: int | None = None, db: AsyncSession = Depends(get_db)
):
...
async def bulk_2fa(body: Bulk2faIn, db: AsyncSession = Depends(get_db)):
    ...
    async def _change(cli, aid):
        ...
        for cand in candidates:
            ...
            try:
                await cli.edit_2fa(
                    current_password=cand, new_password=new_password, hint=hint
                )
                warn = await _save_warn(phone)
                await _set_has_2fa(aid)
                ...
                raise
        raise ValueError(
            f"None of your {tried} current password(s) worked for this account"
        )

    return StreamingResponse(
        bulk_stream(accounts, _change), media_type="application/x-ndjson"
    )
...
async def terminate_others_all(db: AsyncSession = Depends(get_db)):
    ...
    async def _terminate(cli, aid):
        ...
        return ("partial" if failed else "ok"), detail
