...
def _password_hash() -> bytes:
    ...
    if _pw_hash is None:
        ...
        _pw_hash = bcrypt.hashpw(
            settings.APP_PASSWORD.encode(), bcrypt.gensalt(rounds=12)
        )
    return _pw_hash
...
def _get_signer() -> TimestampSigner:
    ...
    if _signer is None:
        ...
        if not secret or len(secret) < 16:
            raise RuntimeError(
                "SESSION_SECRET not set or too short (min 16 chars) in .env"
            )
...
def _client_ip(req: Request) -> str:
    # Do not trust X-Forwarded-For directly. Uvicorn/our trusted edge is
    # responsible for normalizing request.client when proxy headers are allowed.
