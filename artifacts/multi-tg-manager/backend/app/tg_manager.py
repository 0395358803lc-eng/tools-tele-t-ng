...
class TgClientManager:
    def __init__(self):
        ...
        self._pending: dict[
            str, dict
        ] = {}  # phone -> {'client', 'phone_code_hash', 'needs_2fa'}
        self._qr_pending: dict[
            str, dict
        ] = {}  # qr_id -> {'client', 'qr_login', 'wait_task', 'needs_2fa', 'session_path'}
    ...
    def session_file_name(
        self, phone: str, username: str | None = None, user_id: int | None = None
    ) -> str:
    ...
    def _desired_session_path(
        ...
        return str(
            settings.sessions_path / self.session_file_name(phone, username, user_id)
        )
    ...
    def _session_path_candidates(self, acc: Account) -> list[str]:
        ...
        candidates.append(
            self._desired_session_path(acc.phone, acc.username, acc.tg_user_id)
        )
    ...
    async def inspect_imported_session(self, session_base: str) -> tuple[TgUser, str]:
        ...
        try:
            ...
            if not phone:
                raise RuntimeError(
                    "Telegram did not return a phone number for this session"
                )
    ...
    async def promote_imported_session(
        self, session_base: str, phone: str, me: TgUser
    ) -> str:
        dst = self._desired_session_path(
            phone, getattr(me, "username", None), getattr(me, "id", None)
        )
    ...
    async def sync_session_folder(self, force: bool = False) -> dict:
        ...
        async with self._session_scan_lock:
            session_files = [
                p
                for p in sorted(settings.sessions_path.glob("*.session"))
            ...
            for path in session_files:
                ...
                try:
                            ...
                            for base in self._session_path_candidates(acc):
                                try:
                                    candidate_paths.append(
                                        str(Path(base + ".session").resolve()).lower()
                                    )
                                except Exception:
                                    pass
                    ...
                    try:
                        await self.persist_session_blob(acc)
                    except Exception as e:
                        log.warning("session DB snapshot after folder import failed: %s", e)
    ...
    # ---------- lifecycle ----------
    async def rebuild_all_clients_for_api_credentials(self) -> dict[str, object]:
        """Recreate every saved client with the currently active API ID/hash.

        Existing clients are disconnected first so no Telethon object keeps using
        the previous credential pair. Individual session failures are isolated.
        """
        async with AsyncSessionLocal() as db:
                ...
                "results": [
                    {
                        ...
                        "phone": acc.phone,
                        "status": "failed",
                        "detail": "Telegram API is not configured",
                    }
                    for acc in accounts
                ],
            }
        ...
        sem = asyncio.Semaphore(
            min(20, max(1, getattr(settings, "STARTUP_CONCURRENCY", 10)))
        )
        reconnected = 0
        failed = 0
        results: list[dict] = []

        async def _restart(acc: Account):
            nonlocal reconnected, failed
            async with sem:
                try:
                    await self.start_client(acc)
                    reconnected += 1
                    ...
                    failed += 1
                    detail = self._session_error_detail(exc)
                    results.append(
                        {
                            "account_id": acc.id,
                            "phone": acc.phone,
                    ...
                    log.warning("API credential reload failed for %s: %s", acc.phone, exc)

        await asyncio.gather(*(_restart(acc) for acc in accounts))
        results.sort(key=lambda row: int(row["account_id"]))
        return {"reconnected": reconnected, "failed": failed, "results": results}

    async def startup_load_all(self):
        """On boot, restore PostgreSQL-backed sessions and start saved clients."""
        if not (settings.TG_API_ID and settings.TG_API_HASH):
            log.info("Telegram API credentials are not configured; saved clients stay offline")
            return
        async with AsyncSessionLocal() as db:
            res = await db.execute(select(Account))
            accounts = list(res.scalars().all())
        conc = min(50, max(1, getattr(settings, "STARTUP_CONCURRENCY", 10)))
        sem = asyncio.Semaphore(conc)

        async def _start_one(acc: Account):
            async with sem:
                try:
                    base = self._session_path_for_account(acc)
                    mode = await session_store.restore_or_capture(acc.id, base + ".session")
                    if mode != "missing":
                        log.info("session %s for account %s", mode, acc.id)
                    await self.start_client(acc)
                    await session_store.save_file(acc.id, self._session_path_for_account(acc) + ".session")
                except Exception as e:
        ...
        task = asyncio.create_task(_sync_pasted_sessions())
        self._background_tasks.add(task)
        task.add_done_callback(self._background_tasks.discard)

        async def _session_blob_loop():
            while True:
                await asyncio.sleep(120)
                try:
                    await self.sync_session_blobs()
                except Exception as e:
                    log.warning("periodic session DB sync failed: %s", e)

        sync_task = asyncio.create_task(_session_blob_loop())
        self._background_tasks.add(sync_task)
        sync_task.add_done_callback(self._background_tasks.discard)

    async def persist_session_blob(self, acc: Account) -> bool:
        path = self._session_path_for_account(acc) + ".session"
        return await session_store.save_file(acc.id, path)

    async def sync_session_blobs(self) -> int:
        async with AsyncSessionLocal() as db:
        ...
        for acc in accounts:
            ...
            if cli:
                try:
                    ...
                    pass
            try:
                if await self.persist_session_blob(acc):
                    saved += 1
            except Exception as e:
                log.warning("session DB snapshot failed for %s: %s", acc.phone, e)
        return saved
    ...
    async def shutdown(self):
        ...
        try:
            ...
            log.warning("shutdown session DB sync failed: %s", e)
    ...
    async def start_client(self, acc: Account) -> TelegramClient:
        ...
        async with lock:
            ...
            try:
                ...
                log.warning("session snapshot after connect failed for %s: %s", acc.phone, exc)
            # sync profile
            try:
                me = await asyncio.wait_for(cli.get_me(), timeout=30)
    ...
    async def stop_client(self, account_id: int):
        ...
        if cli:
            ...
            try:
                ...
                log.warning("session DB sync on stop failed for %s: %s", account_id, e)

    async def remove_account_instance(
        self, acc: Account, delete_session_file: bool = True
    ):
        await self.stop_client(acc.id)
        if delete_session_file:
            ...
            try:
                ...
                log.warning("remembered 2FA cleanup failed for %s: %s", acc.phone, e)
    ...
    async def send_code(self, phone: str) -> str:
        ...
        if prev:
            try:
                ...
                pass
        cli = TelegramClient(
            self._session_path(phone), settings.TG_API_ID, settings.TG_API_HASH
        )
        await asyncio.wait_for(cli.connect(), timeout=20)
    ...
    async def submit_code(self, phone: str, code: str) -> tuple[TgUser | None, bool]:
        ...
        try:
            await asyncio.wait_for(
                cli.sign_in(
                    phone=phone, code=code, phone_code_hash=pend["phone_code_hash"]
                ),
                timeout=30,
            )
            ...
            try:
                ...
                async with AsyncSessionLocal() as db:
                    exists = await db.scalar(
                        select(SecurityMessage.id).where(
                            SecurityMessage.account_id == account_id,
                            SecurityMessage.tg_msg_id == msg_id,
                        )
                    )
                    if exists:
                        return
    ...
    async def _sync_profile(self, account_id: int, me: TgUser):
        async with AsyncSessionLocal() as db:
            ...
            try:
                ...
                pass
            await db.commit()
        await self._sync_presence(account_id, me)

    async def _sync_presence(self, account_id: int, me: TgUser):
        status = getattr(me, "status", None)
        online = type(status).__name__ == "UserStatusOnline"
        seen = None
        if online:
            ...
            if was_online is not None:
                try:
                    seen = was_online.replace(tzinfo=None)
                except Exception:
                    seen = None
        async with AsyncSessionLocal() as db:
            acc = await db.get(Account, account_id)
            if acc:
                acc.is_online = online
                if seen is not None:
                    acc.last_seen = seen
                await db.commit()

    async def _backfill_777000(
        self, account_id: int, cli: TelegramClient, limit: int = 50
    ):
    ...
    async def refresh_status_all(self):
        ...
        async def _refresh_one(aid: int, cli: TelegramClient):
            async with sem:
                try:
                    ...
                    if ok:
                        try:
                            ...
                            if me:
                                await self._sync_presence(aid, me)
                        except Exception:
                            pass
        ...
        await asyncio.gather(
            *(_refresh_one(aid, cli) for aid, cli in items), return_exceptions=True
        )
