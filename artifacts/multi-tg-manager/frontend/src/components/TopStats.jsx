...
export default function TopStats({ stats, onBellClick }) {
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <Pill label="Tổng"     value={stats.total} />
      <Pill label="Đã kết nối" value={stats.connected} color="bg-brand-ok" />
      <Pill label="Bị cấm"    value={stats.banned}    color="bg-brand-err" />
      ...
      <button onClick={onBellClick} className="relative nb-stat-sm bg-white px-3 py-1 flex items-center gap-2 hover:translate-x-[1px] hover:translate-y-[1px] transition-transform">
        <span className="text-[10px] font-extrabold uppercase">Cảnh báo</span>
