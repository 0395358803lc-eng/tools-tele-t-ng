...
async function request(method, path, { json, form, query, silent } = {}) {
  ...
  try {
    ...
    const e = new Error(viMessage('Cannot reach server'))
    e.status = 0
    e.network = true
    throw e
  }
  ...
  if (!r.ok) {
    ...
    const err = new Error(viMessage(typeof msg === 'string' ? msg : JSON.stringify(msg), r.status))
    err.status = r.status
    err.body = body
    throw err
  }
...
async function streamRequest(path, init, onEvent) {
  ...
  try {
    ...
    const e = new Error(viMessage('Cannot reach server')); e.status = 0; e.network = true; throw e
  }
  if (r.status === 401) _onUnauth.forEach((fn) => { try { fn() } catch {} })
  if (!r.ok || !r.body) {
    let detail = `${r.status} ${r.statusText}`
    try { const b = await r.json(); detail = b?.detail || detail } catch { /* not json */ }
    const err = new Error(viMessage(typeof detail === 'string' ? detail : JSON.stringify(detail), r.status))
    err.status = r.status
    throw err
  }
...
export const Endpoints = {
  ...
  previewRecipientFile: (file) => {
    const fd = new FormData(); fd.append('file', file)
    return api.postForm('/api/messaging/recipient_list_preview', fd)
  },
  sendRecipientFile: (ids, text, file, delayMin, delayMax, onEvent) => {
    const fd = new FormData()
    fd.append('account_ids', ids.join(','))
    fd.append('text', text)
    fd.append('delay_min', String(delayMin))
    fd.append('delay_max', String(delayMax))
    fd.append('file', file)
    return streamNDJSONForm('/api/messaging/recipient_list_send', fd, onEvent)
  },
  ...
  storageStatus: () => api.get('/api/settings/storage_status'),
  putSettings: (s) => api.put('/api/settings', s),
  putTheme: (theme) => api.put('/api/settings/theme', { theme }),
  getTelegramApi: () => api.get('/api/settings/telegram_api'),
  putTelegramApi: (api_id, api_hash = '', reconnect_existing = false) =>
    api.put('/api/settings/telegram_api', { api_id, api_hash, reconnect_existing }),
